...
import {
  ...
  buildForecastPrompt,
  buildSavingsGoalPrompt,
  FinancialHealthPromptInput,
  ForecastPromptInput,
  PromptPayload,
  SavingsGoalPromptInput,
} from '../src/utils/ai-prompts';

interface EvaluationCase<TInput> {
  name: string;
  input: TInput;
  language?: string;
...
interface PromptRunResult {
  output: string | null;
  mode: 'live Gemini output' | 'simulated output';
  errorNote?: string;
}

const forecastCases: EvaluationCase<ForecastPromptInput>[] = [
  {
...
const financialHealthCases: EvaluationCase<FinancialHealthPromptInput>[] = [
  {
...
const savingsGoalCases: EvaluationCase<SavingsGoalPromptInput>[] = [
  {
...
const isRetryableModelError = (err: any): boolean =>
  err?.status === 429 || err?.status === 500 || err?.status === 503;

const formatModelError = (err: any): string => {
...
const validateLiveOutput = (text: string): string | null => {
  const trimmed = text.trim();
  if (!trimmed) return 'Gemini returned an empty response.';
  if (trimmed.length < 60) return `Gemini returned an unexpectedly short response (${trimmed.length} chars).`;
  if (!/[.!?]$/.test(trimmed)) return 'Gemini response did not end with sentence punctuation.';
  const sentenceCount = countSentences(trimmed);
  if (sentenceCount !== 2) return `Gemini response had ${sentenceCount} detected sentences instead of exactly 2.`;
  return null;
};

const runPrompt = async (payload: PromptPayload): Promise<PromptRunResult> => {
...
const renderCase = async <TInput>(
  kind: string,
  evaluationCase: EvaluationCase<TInput>,
  buildPrompt: (input: TInput, language: string) => PromptPayload,
): Promise<string> => {
  const language = evaluationCase.language ?? 'en';
  const payload = buildPrompt(evaluationCase.input, language);
  const promptResult = await runPrompt(payload);
  const output = promptResult.output ?? evaluationCase.simulatedOutput;
...
const main = async (): Promise<void> => {
  ...
  for (const evaluationCase of forecastCases) {
    sections.push(await renderCase('forecast', evaluationCase, buildForecastPrompt));
  }
  for (const evaluationCase of financialHealthCases) {
    sections.push(await renderCase('financial health', evaluationCase, buildFinancialHealthPrompt));
  }
  for (const evaluationCase of savingsGoalCases) {
    sections.push(await renderCase('savings goals', evaluationCase, buildSavingsGoalPrompt));
  }
