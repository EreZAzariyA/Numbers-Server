...
exports.buildSavingsGoalPrompt = exports.buildFinancialHealthPrompt = exports.buildForecastPrompt = void 0;
const asLanguageName = (language) => language === 'he' ? 'Hebrew' : 'English';
const asMoney = (amount) => amount.toFixed(2);
const asSignedMoney = (amount) => `${amount >= 0 ? '+' : '-'}${Math.abs(amount).toFixed(2)}`;
...
const buildForecastPrompt = (input, language) => {
...
const buildFinancialHealthPrompt = (input, language) => {
    const componentEntries = Object.entries(input.components).map(([key, component]) => (Object.assign({ key: key, label: componentLabels[key] }, component)));
...
exports.buildFinancialHealthPrompt = buildFinancialHealthPrompt;
const buildSavingsGoalPrompt = (input, language) => {
...
exports.buildSavingsGoalPrompt = buildSavingsGoalPrompt;
