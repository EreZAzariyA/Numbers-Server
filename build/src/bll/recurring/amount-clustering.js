...
const extractAmount = (tx) => {
    var _a, _b, _c;
    const index = (_a = tx._index) !== null && _a !== void 0 ? _a : 0;
    ...
    const amt = (_c = (_b = tx.chargedAmount) !== null && _b !== void 0 ? _b : tx.amount) !== null && _c !== void 0 ? _c : 0;
    return {
        index,
        amount: Math.abs(amt),
        currency: 'ILS',
...
const clusterAmounts = (txs) => {
    // Tag each tx with its index for traceability.
    const entries = txs.map((tx, i) => {
        const e = extractAmount(Object.assign(Object.assign({}, tx), { _index: i }));
        return e;
    }).filter((e) => e.amount > 0);
    // Group by currency first.
    const byCurrency = new Map();
    for (const e of entries) {
        if (!byCurrency.has(e.currency))
...
exports.clusterAmounts = clusterAmounts;
