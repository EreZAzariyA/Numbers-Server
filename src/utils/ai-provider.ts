...
const generateOllamaInsight = async ({
  ...
  maxOutputTokens,
  model,
}: Pick<GenerateUserInsightArgs, 'prompt' | 'systemInstruction' | 'maxOutputTokens'> & { model: string }): Promise<string> => {
...
export const generateUserInsight = async ({
  ...
  try {
    ...
    if (runtime.provider === 'ollama') {
      response = await generateOllamaInsight({ prompt, systemInstruction, maxOutputTokens, model: runtime.model });
