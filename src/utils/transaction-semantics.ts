...
export const getBillingDate = (transaction: TransactionLike): string =>
  toDateString(transaction.billingDate);
...
export const getCardUniqueId = (transaction: TransactionLike): string =>
  transaction.cardUniqueId ?? transaction.rawTransaction?.cardUniqueId ?? '';

export const inferLegacySemanticType = (transaction: TransactionLike): string => {
...
export const isSemanticSettlement = (transaction: TransactionLike): boolean =>
  getSemanticType(transaction) === 'card_settlement';
...
export const getDateForBasis = (
  transaction: TransactionLike,
  basis: 'event' | 'posting' = 'posting',
): string => {
  return basis === 'event' ? getEventDate(transaction) : getPostingDate(transaction);
};
